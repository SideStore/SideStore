...
final class RefreshAltStoreViewController: UIViewController
{
    ...
    override func viewDidLoad()
    {
        ...
        self.placeholderView.detailTextLabel.text = NSLocalizedString("AppFlex was unable to use an existing signing certificate, so it had to create a new one. This will cause any apps installed with an existing certificate to expire — including AppFlex.\n\nTo prevent AppFlex from expiring early, please refresh the app now. AppFlex will quit once refreshing is complete.", comment: "")
    }
}
            ...
            let group = AppManager.shared.install(altStore, presentingViewController: self, context: self.context) { (result) in
                ...
                case .failure(let error as NSError):
                    DispatchQueue.main.async {
                        ...
                        let alertController = UIAlertController(title: NSLocalizedString("Failed to Refresh AppFlex", comment: ""), message: error.localizedFailureReason ?? error.localizedDescription, preferredStyle: .alert)
