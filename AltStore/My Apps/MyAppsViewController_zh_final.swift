...
private extension MyAppsViewController
{
    ...
    {
        let title = String(format: NSLocalizedString("移除 “%@” from SideStore?", comment: ""), installedApp.name)
        let message: String
    ...
    func importBackup(for installedApp: InstalledApp){
        ImportExport.importBackup(presentingViewController: self, for: installedApp) { result in
            ...
            case .failure(let error):
                ...
                break
            case .success:
                toast = ToastView(text: "导入备份 successful for \(installedApp.name)",
                                  detailText: "Use '恢复备份' option to restore data from this imported backup")
            }
            DispatchQueue.main.async {
                toast.show(in: self)
            }
